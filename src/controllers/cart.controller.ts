  ...
  const itemsWithSubtotal = cart.items.map((item) => {
    ...
    return {
      ...
      room: {
        ...
        primaryImage: item.room.images[0]
          ? { id: item.room.images[0].id, url: item.room.images[0].cloudinaryUrl }
          : null,
      },
    };
  });
...
export const checkoutCart = async (req: Request, res: Response) => {
  ...
  if (conflicts.length > 0) {
    return error(
      res,
      `Some rooms are no longer available: ${conflicts.join(', ')}. Please remove them from your cart.`,
      409
    );
  }
